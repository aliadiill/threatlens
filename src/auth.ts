...
export async function signIn(config: Config) {
  ...
  sessionStorage.setItem("tl-pkce", verifier);
  sessionStorage.setItem("tl-state", state);
  const challenge = b64(
    new Uint8Array(
...
export async function completeSignIn(config: Config) {
  ...
  sessionStorage.removeItem("tl-pkce");
  sessionStorage.removeItem("tl-state");
  history.replaceState({}, "", "/");
  if (!verifier || !state || params.get("state") !== state)
    throw new Error("Sign-in state did not match. Start again.");
  const result = await fetch(new URL("/oauth2/token", config.cognitoDomain), {
    method: "POST",
